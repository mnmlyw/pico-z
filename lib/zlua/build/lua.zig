...
pub fn configure(
    ...
    const flags = [_][]const u8{
        ...
        switch (target.result.os.tag) {
            ...
            .wasi, .freestanding => "", // No POSIX for WASM targets
            else => "-DLUA_USE_POSIX",
        },
    ...
    if (is_wasi) {
        lib.linkSystemLibrary("wasi-emulated-process-clocks", .{});
    ...
    if (is_wasi) {
        // For WASI: exclude loslib.c and liolib.c (use POSIX functions unavailable in WASM)
        // We sandbox these modules anyway in the emulator.
        var wasi_files: std.ArrayList([]const u8) = .empty;
        defer wasi_files.deinit(b.allocator);
        for (lua_source_files) |f| {
            if (!std.mem.eql(u8, f, "src/loslib.c") and !std.mem.eql(u8, f, "src/liolib.c")) {
                wasi_files.append(b.allocator, f) catch @panic("OOM");
            }
        }
        lib.addCSourceFiles(.{
            .root = .{ .dependency = .{ .dependency = upstream, .sub_path = "" } },
            .files = wasi_files.items,
            .flags = &flags,
        });
    } else {
        lib.addCSourceFiles(.{
